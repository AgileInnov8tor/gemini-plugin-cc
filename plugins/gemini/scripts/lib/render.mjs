...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    ...
    lines.push(
      `| ${escapeMarkdownCell(job.id)} | ${escapeMarkdownCell(job.kindLabel)} | ${escapeMarkdownCell(job.status)} | ${escapeMarkdownCell(job.phase ?? "")} | ${escapeMarkdownCell(job.elapsed ?? "")} | ${escapeMarkdownCell(job.geminiSessionId ?? "")} | ${escapeMarkdownCell(job.summary ?? "")} | ${actions.map((action) => `\`${action}\``).join("<br>")} |`
    );
  }
}
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.geminiSessionId) {
    lines.push(`  Gemini session ID: ${job.geminiSessionId}`);
  }
...
export function renderStatusReport(report) {
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh Gemini review and block if it finds issues.");
  }
...
export function renderStoredJobResult(job, storedJob) {
  const sessionId = storedJob?.geminiSessionId ?? job.geminiSessionId ?? null;
  const sessionSuffix = sessionId ? `\nGemini session ID: ${sessionId}\n` : "";

  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    const output = storedJob.rendered.endsWith("\n") ? storedJob.rendered : `${storedJob.rendered}\n`;
    return sessionId ? `${output}${sessionSuffix}` : output;
  }
